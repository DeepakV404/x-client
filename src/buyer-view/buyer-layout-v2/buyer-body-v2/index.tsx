...
import { Button, Layout, Progress, ProgressProps, Space, notification } from 'antd';
import { toLower } from 'lodash';

// import { TOUCH_POINT_TYPE_GENERAL, WHEN_ON_DELAY } from '../../config/buyer-discovery-config';
import { BUYER_RESOURCE, BUYER_UNREAD_NOTIFICATIONS } from '../../api/buyers-query'; //P_TRIGGERED_QUESTIONS
...
import DiscoveryQPopup from '../../layout/buyer-discovery/discovery-q-popup';
import UnreadNotificationsDrawer from '../../layout/buyer-unread-notifications/unread-notifications-drawer';
// import DiscoveryPopup from '../../layout/buyer-discovery/discovery-popup';
import BuyerHomeV2 from './pages/welcome';
import BuyerSectionV2 from './pages/custom-section';
import BuyerFaqV2 from './pages/faq';
import BuyerResourcesV2 from './pages/resources';
import BuyerTalkToUsV2 from './pages/talk-to-us';
import BuyerJourneyV2 from './pages/next-steps';
import BuyerDemoV2 from './pages/demo';

const { Content }   =   Layout;

const BuyerBodyV2 = (props: {setShowPreviewForm: any, isProfileClicked: boolean, setIsProfileClicked: any, setIsIconAnimatingOut: any, isIconAnimatingOut: any, handleMessageOpen: any, journeyStages?: any}) => {
    ...
    const { setShowPreviewForm, handleMessageOpen, journeyStages }   =   props;
    ...
    const hasSectionId = url.searchParams.has("sectionid");        
    ...
    const { data: resourceData } = useQuery(BUYER_RESOURCE, {
        ...
        skip: !hasResourceShareLink || hasSectionId
    });
    ...
    const getRoute = () => {
        ...
        switch (sectionType) {
            ...
            case "RESOURCES":
                return <BuyerResourcesV2/>
            
            case "TALK_TO_US":
                return <BuyerTalkToUsV2/>

            case "FAQ":
                return <BuyerFaqV2 setShowPreviewForm={setShowPreviewForm}/>
        
            default:
                return <BuyerSectionV2/>
        }
    }
    ...
    return (
        <>
            {contextHolder}
            <Content style={{background: "#F5F7F9", height: "unset"}}>
                <Suspense fallback={<Loading />}>
                    <Routes>
                        <Route path="/"                     element={<Navigate to={getNavigateRoute()}/>}/>
                        <Route path="/:stepId"              element={<BuyerJourneyV2 journeyStages={journeyStages}/>}/>
