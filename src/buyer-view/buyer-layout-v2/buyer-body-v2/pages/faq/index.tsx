...
import { TOUCH_POINT_TYPE_FAQ, WHEN_ON_VISIT_FAQ, TOUCH_POINT_TYPE_GENERAL, WHEN_ON_VISIT_FAQ_SECTION } from '../../../../config/buyer-discovery-config';
import { BuyerDiscoveryContext } from '../../../../context/buyer-discovery-globals';

const BuyerFaqV2 = (props: {setShowPreviewForm: any}) => {
    ...
    return( 
        <div className="bn-section-body-wrapper" style={{minHeight: "calc(100vh - 410px)"}}>
            <div className="bn-section-container">
                <div className='j-buyer-cm-card cm-width100 cm-flex cm-flex-direction-column cm-flex-align-center cm-width100' style={{rowGap: "25px"}}>
                    <Space direction='vertical' align='center' className='cm-width100'>
                        ...
                        <div className='j-faq-subtitle cm-font-fam400 cm-font-opacity-black-65'>
                            <Translate i18nKey='faq.subtitle'/>
                        </div>
                    </Space>
                    <Collapse 
                        style               =   {{background: "#fff"}}
                        className           =   'j-faq-collapse cm-width100'
                    ...
                    >
                        {
                            data.faqs.map((_faq: { question: string; uuid: string; answer: string }) => (
                                <CollapsePanel 
                                    header    =  {<div style={{wordBreak: "break-word"}} className='cm-font-size15'>{_faq.question}</div>}
                                    key       =  {_faq.uuid}
                                >
