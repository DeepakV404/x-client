...
import { Button, Form, Input, Space, Tooltip, Upload } from 'antd';
...
const AddResources = (props: {setEditView: (arg0: boolean) => void, currentStage: string}) => {
    ...
    const getFileCard = () => {
        if(selectedResource || uploadedFile || pastedLink){
            if(selectedResource){
                return(
                    <Space className='cm-width100 j-uploaded-pitch-card cm-flex-space-between'>
                ...
                return (
                    <Space className='cm-width100 j-uploaded-pitch-card cm-flex-space-between'>
                        <div className='cm-font-fam500'>{pastedLink}</div>
