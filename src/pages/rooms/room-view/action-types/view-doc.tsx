...
const ViewDoc : FC<ViewDocProps> =  (props) => {
    ...
    const onFinish = () => {
        ...
        if(pastedLink){
            ...
            updateResource(selectedResource.uuid)
        }
    }
            ...
            <Space direction='vertical' className='cm-width100' size={15}> 
                {
                    defaultActionData?.resources ?
                        defaultActionData.resources.map((_resource: any) => (
                            <Card className="j-action-download-card" hoverable key={_resource.resource.uuid} onClick={() => console.log(_resource.resource)}>
                                <Space className="cm-flex-space-between">
                                    ...
                                    <Space size={10}>
                                        ...
                                        <div  onClick={(event) => {event.stopPropagation(); window.open(_resource.resource.content.url)}}>
                                            <MaterialSymbolsRounded font="download" size="22" className="cm-cursor-pointer"/>
                                        </div>
                                    </Space>
                                </Space>
                            </Card>
                    ...
                    :
                        defaultActionData?.link &&
                            <Card className="j-action-download-card" hoverable key={defaultActionData?.link} onClick={() => window.open(defaultActionData?.link, "_blank")}>
                                <Space className="cm-flex-space-between">
                                    <Text className="j-action-text cm-font-size14 cm-font-fam400" ellipsis={{tooltip: defaultActionData?.link}} style={{maxWidth: "450px"}}>{defaultActionData?.link}</Text>
                                </Space>
                            </Card>
                }
                {
                    ...
                    <Space direction="vertical" className="cm-width100 cm-space-inherit cm-flex-center" size={25} >
                        ...
                        <Space direction="vertical" className="cm-width100 cm-space-inherit cm-flex-center" size={10} >
                            ...
                            <Form.Item name="blobFile" noStyle>
                                <Dragger beforeUpload={()=> {return false}} onChange={handleUpload}  showUploadList={false} disabled={selectedResource || pastedLink} accept={acceptedFileTypes}>
