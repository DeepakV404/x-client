...
const Regions = () => {
    ...
    const { data, loading, error } = useQuery(REGIONS, {
            ...
            PermissionCheckers.__checkUsecaseCategoryPermisson($user.role).update ?
                {
                    ...
                    onClick: () => {
                        setShowModal({ visible: true, type: { name: "Region", op: "edit" }, currentResourceCategory: category })
    ...
    const handleDelete = (category: any) => {
        SettingsAgent.deleteRegion({
            variables: {
                uuid: category.uuid
            },
            onCompletion: () => {
                CommonUtil.__showSuccess("Region deleted successfully");
        ...
        <>
            <div className="cm-height100">
                <div className="j-setting-header cm-flex-space-between-center" style={{ height: "53px" }}>
                    <div className="cm-font-size16 cm-font-fam500">Regions</div>
                    {
                        PermissionCheckers.__checkUsecaseCategoryPermisson($user.role).create
                        ?
                            <Button type='primary' className="j-setting-footer-btn cm-flex-center" onClick={() => setShowModal({ visible: true, type: { name: "Region", op: "add", currentResourceCategory: null } })}>
                                Add
                            </Button>
                        :
                            null
                        ...
                        ?
                            (
                                data.regions.length > 0 ?
                                    data.regions.map((item: { uuid: string, name: string }) => (
                                        item.uuid !== "all-regions" &&
                                ...
                                :
                                    <div className='cm-empty-text cm-flex-center' style={{ height: "calc(100vh - 160px)" }}>No Regions found</div>
                            )
                        :
            ...
            <DeleteConfirmation
                ...
                body    =   "Are you sure you want to delete this region? This cannot be undone."
            />
        </>
    )
}
