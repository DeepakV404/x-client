...
import { RESOURCE } from '../../pages/library/api/library-query';
...
const ResourceViewerLayout = (props: {inProduct?: boolean, resourceId: string,  fileInfo: any}) => {
    
    const { fileInfo, resourceId, inProduct=true }  =   props;

    const fileViewerRef = useRef();

    const { data, loading, error }  =   useQuery(RESOURCE, {
    ...
    let $fileInfo   =   data.resource;
    let $report     =   data.resource.report;
    let $roomReport =   data.resource.roomReports;
    ...
    return (
        <Row gutter={20} className="cm-height100">
            <Col span={16} className='cm-height100'>  
                {getFileViewerComponent($fileInfo)}
            </Col>
            <Col span={8} style={{height: inProduct ? "calc(100vh - 90px)" : "calc(100vh - 40px)", overflow: "auto"}}>
                ...
                <Row gutter={inProduct ? [20, 20] : [10, 5]} className={inProduct ? '' : "cm-margin0"}>
                    ...
                    <Col span={24} className={inProduct ? '' : "cm-padding-left0"}>
                        <Card className={inProduct ? '' : ''}>
                            <Statistic
                                ...
                                value       =   {$report.lastViewedAt ? `${CommonUtil.__getDateDay(new Date($report.lastViewedAt))}, ${new Date($report.lastViewedAt).getFullYear()} - ${CommonUtil.__format_AM_PM(new Date(fileInfo.report.lastViewedAt))}` : "-"}
                                precision   =   {2}
                                valueStyle  =   {{ color: '#000', fontSize: inProduct ? "22px" : "15px" }}
                                className   =   'cm-font-fam500'
                            />
                    ...
                    <Col span={24} className={inProduct ? '' : "cm-padding-left0"}>
                        ...
                        {
                            $roomReport.length > 0 ?
                                $roomReport.map((_roomReport: any) => (
                                    _roomReport.report.viewDetails.length > 0 &&
                                        <List
                                            bordered
                                            itemLayout  =   "horizontal"
                                            className   =   {inProduct ? 'cm-margin-bottom20' : 'cm-margin-bottom5'}
                                            header      =   {<Space size={4} style={{display: "flex", alignItems: "start"}}><div className={inProduct ? 'cm-font-fam500' : 'cm-font-fam500 cm-font-size10'}>Room: </div> <div className={inProduct ? 'cm-font-fam600 cm-font-size15' : 'cm-font-fam600 cm-font-size12'}>{_roomReport.roomStub.title}</div></Space>}
                                            dataSource  =   {_roomReport.report.viewDetails}
...
export default ResourceViewerLayout